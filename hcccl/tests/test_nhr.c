/** @file test_nhr.c — Unit tests for nhr_allreduce CPU simulation. */
...
static float go(hcclComm_t c, int32_t r, float v, float *o){
    ...
    nhr_allreduce(&v,&recv,1,HCCL_FP32,HCCL_SUM,c);
    if(o)*o=recv; return recv;}

static void t4(void){
    T("4 ranks [1,2,3,4] → 10");
...
static void tns(void){
    T("NULL send_buf → INVALID_ARG");
    int32_t ids[]={0}; hcclComm_t c=NULL; hcclCommInit(&c,1,ids); hcclSetRank(c,0);
    float r;
    if(nhr_allreduce(NULL,&r,1,HCCL_FP32,HCCL_SUM,c)==HCCL_ERR_INVALID_ARG) OK();
    else NO("expected INVALID_ARG"); hcclCommDestroy(c);}

static void tnr(void){
    T("NULL recv_buf → INVALID_ARG");
    int32_t ids[]={0}; hcclComm_t c=NULL; hcclCommInit(&c,1,ids); hcclSetRank(c,0);
    float s=1;
    if(nhr_allreduce(&s,NULL,1,HCCL_FP32,HCCL_SUM,c)==HCCL_ERR_INVALID_ARG) OK();
    else NO("expected INVALID_ARG"); hcclCommDestroy(c);}
